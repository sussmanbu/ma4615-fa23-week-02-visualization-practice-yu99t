...
The analysis after grouping by race and state does not proceed due to lacking my skill to create a data set that merges the columns so that the number of cases and deaths are added together if the race and region are the same on each date.
...
In addition, when considering the burden on medical institutions, we should collect not only the number of infections, but also the number of severely ill patients, which is important when considering policies for the maintenance and support of medical institutions.
